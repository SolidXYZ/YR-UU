...
function getAutocompleteURL(str, types = 'all', lat = 0, lon = 0, strictbounds = false) {
    return `https://cors-anywhere.herokuapp.com/` + getGeocodeURL(`place/autocomplete/json?input=${str}&types=${types}&location=${lat.toString()}%2C${lon.toString()}&strictbounds=${strictbounds.toString()}`);
}
