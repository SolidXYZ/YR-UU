    ...
    */
    ...
    searchTextInput.oninput = () => {
        if (searchTextInput.value.length >= 5) {
            if (searchingForGeoLocation === false) {
                searchSelectInput.innerHTML = searchSelectInput.options[0];
                searchingForGeoLocation = true;
                getRawGeocodeLocation(searchTextInput.value)
                    .then((data) => {
                        ...
                        if (data !== undefined) {
                            ...
                            if (res !== undefined) {
                                ...
                                for (let i = 0; i < res.length; i++) {
                                    ...
                                    opt.innerHTML = optData.formatted_address;
    ...
    searchForm.onsubmit = (e) => {
        ...
        if (isOptionFirst(searchSelectInput) === false) {
            const data = curOptions[Number(searchSelectInput.value)];
...
function getGeocodeLocationURL(loc, fields = ['formatted_address', 'geometry']) {
    return getGeocodeURL(`place/findplacefromtext/json?fields=${arrToASCII(fields)}&input=${strToASCII(loc)}&inputtype=textquery`);
}
function getAutocompleteURL(str, types = 'all', lat = 0, lon = 0, strictbounds = false) {
    return getGeocodeURL(`place/autocomplete/json?input=${str}&types=${types}&location=${lat.toString()}%2C${lon.toString()}&strictbounds=${strictbounds.toString()}`);
}
...
function fetchGeocodeLocation(...args) {
    return fetchWithTimeout('https://cors-anywhere.herokuapp.com/' + getGeocodeLocationURL(...args));
}
...
function fixData(periodsData) {
    for (const [periodName, periodData] of Object.entries(periodsData)) {
        let i;
        if (periodName === 'instant') {
            i = 0;
        } else {
            i = timePeriods.indexOf(Number(periodName.replace('next_', '').replace('_hours', ''))) + 1;
        }
        delete periodsData[periodName];
        periodsData[i] = periodData;
    }
}
function fixAllData(arr) {
    for (const period of arr) {
        fixData(period.data);
    }
}
function getDataAtTime(timeInMS, arr) {
    let res;
    for (let i = 0; i < arr.length; i++) {
        const cur = arr[i];
    ...
    return new Promise((resolve, reject) => {
        getRawData(type, lat, lon, alt)
            .then((data) => {
                ...
                let res;
                if (filterType === 'basic') {
                    res = getDataAtTime(timeInMS, arr);
                    ...
                    for (const period of timePeriods) {
                        fillInMissingData(data[i], getDataAtTime(timeInMS + (period * 60 * 60 * 1000), arr).data[0]);
                        i += 1;
                    }
